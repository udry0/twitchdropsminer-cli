...
A basic headless mode is available for Linux/server-style setups.
...
A reusable launcher script is included:

```bash
./run_headless.sh
```

For long-running service usage:

- Environment example: `contrib/twitchdropsminer.env.example`
- systemd unit template: `contrib/twitchdropsminer@.service`
...
sudo cp /opt/twitchdropsminer/contrib/twitchdropsminer.env.example /etc/default/twitchdropsminer
sudo systemctl daemon-reload
sudo systemctl enable --now twitchdropsminer@$USER
sudo journalctl -u twitchdropsminer@$USER -f
```

Notes:
- Cookies and settings stay in the repo/app working directory, same as the GUI build.
- The first login in headless mode uses Twitch device-code auth and still needs manual confirmation in a browser.
- Running the browser player on the same Twitch account while mining can still break drop progress.
